...
static	void ( *toneadj ) (void); /*  Tone adjustment on clock tick   */
...
static  unsigned  soundrand( unsigned int modulo );
...
void swsound(void)
{
	...
	switch ( soundtype ) {
		...
		case S_HIT:
			tone( soundrand( 2 ) ? 0x9000 : 0xF000 );
			lastobj = NULL;
			toneadj = NULL;
			break;
